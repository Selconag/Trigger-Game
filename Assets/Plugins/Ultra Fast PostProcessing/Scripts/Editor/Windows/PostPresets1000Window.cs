...
namespace EM.PostProcessing.Editor
{
	...
	public interface ISupportedPostComponent
	{
		...
		void SetMonoComponentDefaultParameters(); // default parameters for added new component
												 // - additional components
		Type SecondMonoComponentType { get; }
		MonoBehaviour SecondMonoComponent { get; set; } // cached component for the current frame
		void SetSecondMonoComponentDefaultParameters(); // default parameters for added new component
	...
	{   new PostPresets_DefaultFastMobile()
        //, new PostPresets_UnityPostGUI_v2()
	};
	}
	...
	public partial class Params
	{
        ...
        public const string TITLE = "Post 99+";
#else
		public static Type WindowType = typeof(PostPresets1000Window);
		public const string TITLE = "Post 1000+ Luts Pack";
		public const string TOPMENU = FastPostProcessingProfile.MENU_PATH + "LUTs";
#endif

		public const string STANDARD_FOLDER = "LUTs_Default_Textures";
		public const string USER_FOLDER = "LUTs_User_Textures";
		...
		public const string MenuItems0 = "Export All Camera's settings as one LUT.png (You can use this option with another post processing asset)";
		public const string MenuItems0_INV = "Export2DLut";
		public const string MenuItems1 = "Export All Camera's settings - test feature tries to read whole camera color settings and tries to write them in the one LUT file";
...
#pragma warning restore
		...
		//tatic bool searchForEditorResourcesPath( out string path )
		//
...

	{
		...
		[MenuItem( Params.TOPMENU + "/LUTs Manager", false, FastPostProcessingProfile.MENU_ORDER + 499)]
		public static EditorWindow Init()
		{
			...
			if ( !EditorPrefs.GetBool( "EMX/" + Params.TITLE + "/Init", false ) )
			{
				EditorPrefs.GetBool( "EMX/" + Params.TITLE + "/Init", true );
			...
			var k = "EMX/" + Params.TITLE + "/ScreenShot";
		...
		const string only = "";
#endif
		[MenuItem( Params.TOPMENU + expcnv + Params.MenuItems0 + only, false, FastPostProcessingProfile.MENU_ORDER + 500 )]
...
#if P99
        [MenuItem(Params.TOPMENU + expcnv + Params.MenuItems0 + only, true, FastPostProcessingProfile.MENU_ORDER + 500)]
        public static bool MenuItems0v() { return false; }
#endif
		/*  [MenuItem(Params.TOPMENU + expcnv + Params.MenuItems1 + only, false, FastPostProcessingProfile.MENU_ORDER + 501)]
		  public static void MenuItems1() { }
		  [MenuItem(Params.TOPMENU + expcnv + Params.MenuItems1 + only, true, FastPostProcessingProfile.MENU_ORDER + 501)]
		  public static bool MenuItems1v() { return false; }*/
		[MenuItem( Params.TOPMENU + expcnv + Params.MenuItems2 + only, false, FastPostProcessingProfile.MENU_ORDER + 502 )]
...
#if P99
        [MenuItem(Params.TOPMENU + expcnv + Params.MenuItems2 + only, true, FastPostProcessingProfile.MENU_ORDER + 502)]
        public static bool MenuItems2v() { return false; }
#endif
		[MenuItem( Params.TOPMENU + expcnv + Params.MenuItems3 + only, false, FastPostProcessingProfile.MENU_ORDER + 503 )]
...
#if P99
        [MenuItem(Params.TOPMENU + expcnv + Params.MenuItems3 + only, true, FastPostProcessingProfile.MENU_ORDER + 503)]
        public static bool MenuItems3v() { return false; }
#endif
		[MenuItem( Params.TOPMENU + expcnv + Params.MenuItems4 + only, false, FastPostProcessingProfile.MENU_ORDER + 504 )]
...
#if P99
        [MenuItem(Params.TOPMENU + expcnv + Params.MenuItems4 + only, true, FastPostProcessingProfile.MENU_ORDER + 504)]
		...
		void ResetWindow()
		{
			...
			EditorPrefs.SetInt( "EMX/" + Params.TITLE + "/Scene", SceneManager.GetActiveScene().GetHashCode() );
...
#pragma warning restore
		...
		string[] sotrNames = new string[] { "Name"
            #if !P99
            , "Saturate", "Warm", "Bright" 
            #endif
...
//        static bool IsVolumeRenderedByCamera(UnityEngine.PostProcessing.PostProcessVolume volume, Camera camera)
//        {
...
//#endif
		...
		void _OnGUI()
		{
			...
			if ( SceneManager.GetActiveScene().GetHashCode() != EditorPrefs.GetInt( "EMX/" + Params.TITLE + "/Scene", -1 ) ) ResetWindow();
			...
			int NEW_POP_A;
			{
				...
				restoreEnables2 = allCommps.Where( c => c ).Distinct().ToDictionary( c => { return c; }, c => { return c.enabled; } );
...
//#endif
			...
			if ( currentComponent.SecondMonoComponentType != null && currentComponent.SecondMonoComponent == null )
			{
				...
				if ( GUILayout.Button( "Press - to add " + compName + " script", GUILayout.Height( 200 ) ) )
				{
					...
					if ( sc )
					{
						EditorUtility.DisplayDialog( "Press - to add " + compName + " script", "Unable to use Scene Camera.\n\nFirst select your game camera, add a script, and only then turn on the Scene Camera.", "Ok" );
					}
					else
					{
						Undo.RecordObject( Params.camera.gameObject, "Press - to add " + compName + " script" );
								...
								&& currentComponent.SecondMonoComponentType != null )
						{
							...
							Undo.RegisterCreatedObjectUndo( currentComponent.SecondMonoComponent, "Press - to add " + compName + " script" );
						}
						if ( currentComponent.SecondMonoComponent ) currentComponent.SetSecondMonoComponentDefaultParameters();
...
//#endif
				...
				if ( GUILayout.Button( "Press - to add " + compName + " script", GUILayout.Height( 200 ) ) )
				{
					...
					if ( sc )
					{
						EditorUtility.DisplayDialog( "Press - to add " + compName + " script", "Unable to interact with Scene Camera.\n\nFirst select your game camera, add a script, and only then turn on the Scene Camera.", "Ok" );
					}
					else
					{
						Undo.RecordObject( Params.camera.gameObject, "Press - to add " + compName + " script" );
						if ( currentComponent.MonoComponent == null )
						{
							...
							if ( currentComponent.MonoComponent ) Undo.RegisterCreatedObjectUndo( currentComponent.MonoComponent, "Press - to add " + compName + " script" );
						}

						if ( currentComponent.MonoComponent ) currentComponent.SetMonoComponentDefaultParameters();
			...
			{
				GUILayout.BeginHorizontal();
				if ( Params.TransparentButton( "HELP: https://emem.store/wiki?=PostPresets 1000+", GUILayout.Width( leftwidth ) ) ) Application.OpenURL( "https://emem.store/wiki?=PostPresets%201000+" );
				EditorGUIUtility.AddCursorRect( GUILayoutUtility.GetLastRect(), MouseCursor.Link );
				//  if ( Params.TransparentButton( "Startup Window", GUILayout.Width( leftwidth ) ) ) Application.OpenURL( "https://emem.store/wiki?=PostPresets%201000+" );
				GUILayout.EndHorizontal();
			}
		...
		void FiltresAndSorting( EditorWindow window, float leftWidth )
		{
			...
			// r.width -= 20;
			GUI.Label( r, "Sorting:" );
			for ( int i = 0; i < sotrNames.Length; i++ )
			{
				var currentSort = Mathf.Clamp(Params.sortMode, 0, sotrNames.Length - 1);
				var sortNotInverse = ((int)Params.sortInverse & (1 << i)) == 0;
				if ( Button( GetRect( leftWidth, 20, window, D ), sotrNames[ i ] + (currentSort == i ? (sortNotInverse ? "▼" : "▲") : ""), currentSort == i, "Set Ordering Method" ) )
				{
					if ( currentSort == i )
					{
		...
		void DrawPresets( ISupportedPostComponent component )
		{
			...
			if ( __M == -1 )
			{
				if ( Params.showFav == 1 ) GUILayout.Label( "No Favorite Elements", Params.Label );
					...
					if ( Params.showFav == 2 ) GUILayout.Label( "No User's Elements. Place your png into " +  Params.USER_FOLDER, Params.Label );
				else GUILayout.Label( "No elements '" + (string.IsNullOrEmpty((string)Params.filtres) ?(  Params.STANDARD_FOLDER) : (string)Params.filtres) + "' found", Params.Label );
			}
			else
			{
...
//#endif
				...
				try
				{
					...
					//change
					if (component.LutEnable != 1) component.LutEnable = int.MaxValue;
...
//#endif
		...
		{   sRGB = false, width = resWidth, height = resHeight, depthBufferBits = 24, volumeDepth = 1,
			...
			screenShot.name = "EMX/" + Params.TITLE + "/ScreenShot";
			screenShot.Apply();
			return screenShot;
		}
...
#pragma warning restore
	...
	public class CachedInt
	{
		...
		{
			this._key = "EMX/" + Params.TITLE + "/" + key;
			this._value = null;
